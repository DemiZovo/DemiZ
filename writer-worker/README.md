...
草稿箱使用浏览器 IndexedDB 保存文章字段和图片，不写入 GitHub。“保存草稿”更新当前记录，“另存为新草稿”保留原记录并创建副本，“新建文章”清空编辑器但不会删除草稿箱内容。导入 Markdown 时会读取文件开头的 frontmatter，下载 Markdown 时会自动生成完整 frontmatter。
